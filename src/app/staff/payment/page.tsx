...
export default function StaffDashboard() {
  ...
  async function fetchTickets() {
    ...
    const calledTicket = data.find(
      (ticket: Ticket) => ticket.status === "CALLED" && ticket.prefix === "P"
    );
    setCalledTicketId(calledTicket ? calledTicket.id : null);
    const servingTicket = data.find(
      (ticket: Ticket) => ticket.status === "SERVING" && ticket.prefix === "P"
    );
  ...
  async function callNextTicket() {
    const nextTicket = tickets.find(
      (ticket) => ticket.status === "PENDING" && ticket.prefix === "P"
    );
    if (nextTicket) {
